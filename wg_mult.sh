...
function check_selinux(){
    ...
    if [ "$CHECK" == "SELINUX=enforcing" ]; then
        red "======================================================================="
        red "檢測到SELinux為開啟狀態，為防止wireguard連接失敗，請先重啟VPS後，再執行本腳本"
        red "======================================================================="
        read -p "是否現在重啟 ?請輸入 [Y/n] :" yn
	    [ -z "${yn}" ] && yn="y"
	    if [[ $yn == [Yy] ]]; then
    ...
    if [ "$CHECK" == "SELINUX=permissive" ]; then
        red "======================================================================="
        red "檢測到SELinux為寬容狀態，為防止wireguard連接失敗，請先重啟VPS後，再執行本腳本"
        red "======================================================================="
        read -p "是否現在重啟 ?請輸入 [Y/n] :" yn
	    [ -z "${yn}" ] && yn="y"
	    if [[ $yn == [Yy] ]]; then
	...
	iptables -P INPUT ACCEPT
 	...
 	iptables -F
  	...
  	service iptables restart
    	...
    	red "==================="
        red "暫未支援ubuntu19.04系統"
        red "==================="
    elif [ "$RELEASE" == "ubuntu" ]  && [ "$VERSION" == "19.10" ]; then 
    	red "==================="
        red "暫未支援ubuntu19.10系統"
        red "==================="
...
EOF
    ...
    green "電腦端請下載/etc/wireguard/client.conf檔，手機端可直接使用軟體掃碼"
...
function add_user(){
    ...
    green "=================================="
    green "給新用戶起個名字，不能和已有用戶重複"
    green "=================================="
    read -p "請輸入用戶名：" newname
    cd /etc/wireguard/
...
EOF
    	...
    	green "============================================="
    	green "添加完成，檔：/etc/wireguard/$newname.conf"
    	green "============================================="
