...
if [ -f "/etc/selinux/config" ]; then
    ...
    if [ "$CHECK" != "SELINUX=disabled" ]; then
        green "檢測到SELinux開啟狀態，添加開放80/443連接埠規則"
...
if [ "$firewall_status" == "running" ]; then
    green "檢測到firewalld開啟狀態，添加放行80/443連接埠規則"
...
if [ -n "$Port80" ]; then
    ...
    red "==========================================================="
    red "檢測到80埠被佔用，佔用進程為：${process80}，本次安裝結束"
    red "==========================================================="
    exit 1
fi
if [ -n "$Port443" ]; then
    ...
    red "============================================================="
    red "檢測到443埠被佔用，佔用進程為：${process443}，本次安裝結束"
    red "============================================================="
    exit 1
fi
}
function install_nginx(){
    ...
    green "開始編譯安裝nginx，編譯等待時間可能較長，請耐心等待，通常需要幾到十幾分鐘"
    sleep 3s
    make >/dev/null 2>&1
...
function install(){
    ...
    if [ $real_addr == $local_addr ] ; then
        green "=========================================="
	green "         功能變數名稱解析正常，開始安裝"
	green "=========================================="
        install_nginx
    else
        red "===================================="
	red "功能變數名稱解析位址與本VPS IP位址不一致"
